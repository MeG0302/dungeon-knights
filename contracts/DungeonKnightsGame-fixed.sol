 ...
 * @dev TRUE BATCH CLAIMING - Claims all dungeons in one transaction with no cooldown
 */
contract DungeonKnightsGame is Ownable {
    ...
    /**
     * @notice Complete dungeon and claim rewards (single claim)
     */
    function completeDungeon(
        ...
        require(timeSpent >= dungeons[dungeonId].minCompletionTime, "Completion too fast");

        // Anti-cheat: maximum reward check
        require(reward <= dungeons[dungeonId].maxRewardPerRun, "Reward too high");

        // Verify signature
        bytes32 messageHash = keccak256(abi.encodePacked(
            knightId, dungeonId, timeSpent, reward, timestamp, msg.sender
        ));

        bytes32 ethSignedHash = MessageHashUtils.toEthSignedMessageHash(messageHash);
        require(ECDSA.recover(ethSignedHash, signature) == msg.sender, "Invalid signature");
        ...
        // Emit event
        emit DungeonCompleted(msg.sender, knightId, dungeonId, timeSpent, reward, timestamp);

        // Transfer DNG rewards
     ...
     * @dev OPTIMIZED: Reduced local variables to avoid stack too deep error
     */
    function batchCompleteDungeons(
        ...
        for (uint i = 0; i < dungeonIds.length; i++) {
            // Validate dungeon
            require(dungeons[dungeonIds[i]].active, "Dungeon not active");
            require(timestamps[i] <= block.timestamp, "Future timestamp");
            require(timeSpents[i] >= dungeons[dungeonIds[i]].minCompletionTime, "Completion too fast");
            require(rewards[i] <= dungeons[dungeonIds[i]].maxRewardPerRun, "Reward too high");

            // Verify signature
            bytes32 messageHash = keccak256(abi.encodePacked(
                knightId, dungeonIds[i], timeSpents[i], rewards[i], timestamps[i], msg.sender
            ));

            bytes32 ethSignedHash = MessageHashUtils.toEthSignedMessageHash(messageHash);
            require(ECDSA.recover(ethSignedHash, signatures[i]) == msg.sender, "Invalid signature");

            // Accumulate rewards
            totalReward += rewards[i];

            // Emit event for each completion
            emit DungeonCompleted(msg.sender, knightId, dungeonIds[i], timeSpents[i], rewards[i], timestamps[i]);
        }
