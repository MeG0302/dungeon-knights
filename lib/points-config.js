...
/**
 * Where a winner hands over the address the capsule should go to.
 *
 * A Google Form for now, in an env var because that link will change: the one published at launch
 * is a **restricted** form, so most players clicking it meet a Google sign-in rather than a form,
 * and the fix is to publish it and swap one Vercel value rather than ship a release. The page says
 * plainly what the link needs, and the Discord invite beside it is the path for anyone who cannot
 * open it.
 */
export const CAPSULE_FORM_URL = (process.env.CAPSULE_FORM_URL
    || 'https://docs.google.com/forms/d/18i-Zn5UxiycTdTVak0H9U8p2H0TIm3fFftEwntkCbqs/edit').trim();
