...
import {
    ...
    commission, todayKey, streakFor, referralQualifies, REFERRAL_MIN_POINTS,
    SITE_LINK_HOST, X_CAMPAIGN_KEYWORD, X_CAMPAIGN_POST, X_ENGAGEMENT_REWARD,
    X_VERIFY_MAX_ATTEMPTS, X_VERIFY_MIN_GAP_SECONDS,
    X_SHARE_TAG, SHARE_OG_IMAGE, dayMarker, programDay,
    campaignPostId, campaignTaskId, shareTaskId,
    sharePostText, shareIntentUrl, referralUrl, refTokenFromInput,
    ONE_TIME_TASKS, oneTimeTask, oneTimeTaskByKind,
    countsTowardDaily,
} from './points-config.js';
import {
    STORAGE_DRIVER, blankWallet, bumpDailyPoints, claimGuard, getWallet, normaliseAddress,
    rankOf, topWallets, updateWallet, walletCount, claimXBinding, releaseXBinding, followFact,
    pendingAdd, pendingRemove, pendingList,
    assignRefCode, refCodeOwner,
} from './points-store.js';
import { settleDraws } from './points-draw.js';
import { giveawayView } from './points-capsules.js';
import { parseStatusUrl, statusIdFromUrl, verifyPost } from './x-verify.js';
import { followProofMode } from './x-webhook.js';
import { extraOneTimeTasks, findExtraTask } from './points-tasks.js';
...
/**
 * How far up a referral chain the cycle check will walk before giving up.
 *
 * Commissions are only ever paid two levels deep, so anything longer than a handful of hops is
 * already a corrupted record rather than a real chain — the bound exists so that record cannot turn
 * a claim into an endless loop.
 */
const REF_CHAIN_LIMIT = 8;
 ...
 * it, and one that is not credits the claim on review. `claimedProof` is what was actually true the
 * day the claim was paid, which does not move when the wiring changes: a claim settled from the
 * review queue keeps saying that, rather than being retroactively relabelled as checked.
 *
 ...
 */
/**
 * The one-time tab, as this wallet should see it.
 *
 * A task that has been **paid** leaves the tab. That is the owner's rule and the reason is a good
 * one: the tab should be about what is still worth doing, not a wall of what is not. The two tasks
 * that pay on a claim are the exception — the follow, and the waitlist — and they stay, marked
 * claimed, for the reason the follow always did: they are the two tasks **every** player has, and a
 * card that disappears for some players and not others reads as a bug rather than as a receipt. A
 * settled claim card is also the only place either of them says what was and was not checked, and
 * the waitlist's answer (“nothing was checked”) is worth keeping in front of a player rather than
 * letting it vanish the moment it is paid.
 *
 * The four quote-reposts still go when they are paid: they are individual posts on a campaign that
 * will keep adding them, so they are the ones that must not pile up.
 *
 * This is presentation and nothing else. The record stays on the wallet, `claimOneTime` and
 * `submitTask` both still refuse a second payout, and the leaderboard counts the points either way:
 * a hidden task is a paid task.
 */
function oneTimeTasks(doc, extras = []) {
    return [...ONE_TIME_TASKS, ...extras].filter((task) => {
        if (task.proof === 'claim' || task.proof === 'visit') return true;
        return doc?.tasks?.[`one:${task.id}`]?.state !== 'verified';
    });
}
    ...
    return {
        ...
        claimedNote: !claimed
            ...
            : (claimedProof === 'webhook'
                ...
                : (claimedProof === 'visit'
                    // The receipt for a task that settled with nothing behind it. The follow's says a
                    // review decided it and a checked task says X was asked; this one says what
                    // happened — it was credited on the claim — and makes no claim about a check
                    // either way, which is the same rule the card's own sentence follows.
                    ? 'credited on claim'
                    : (record.review === true
                        ? (record.reviewedBy === 'tool' ? 'approved in review' : 'credited after review')
                        : 'credited when claimed'))),
 ...
 * repost carries our site link, a share tags our account **and carries today's day marker**. One
 * verifier, two rewards, and no second place for the rules to drift to; what differs is the amount,
 * the requirement, and how often it pays.
 */
function taskConfig(kind, doc, day = todayKey(), extras = []) {
    ...
    // The four that shipped, then the ones the owner has published since — both are the same
    // bargain, so both are settled by the code below without knowing which list they came from.
    const oneTimePost = oneTimeTaskByKind(kind) || findExtraTask(extras, kind);
    if (oneTimePost) {
    ...
    if (kind === 'share') {
        ...
        return {
            ...
            //
            // The day marker is new to this list, and it is what the daily limit actually rests on:
            // the share pays **once a day**, so without it the cheapest way to be paid twice for one
            // post is to file yesterday's link again today. `dayMarker` writes the same string the
            // post was composed with (`sharePostText`), so the two cannot drift.
            expect: { handle, mention: X_SHARE_TAG, marker: dayMarker(day) },
        };
    }
...
export async function stateFor(address) {
    ...
    // The same bargain for the daily capsule draw: any day whose board has closed but has no record
    // yet is settled by the read that is about to render it. The cron endpoint makes the common case
    // prompt; this is what makes the feature correct without one.
    await settleDraws();
    const doc = (await getWallet(key)) || blankWallet(key);
    // Minting the code here, in the read that renders it, is what gives one to the wallets that were
    // already playing before codes existed — they never take the join path again.
    const refCode = doc.refCode || await assignRefCode(key);
    const day = todayKey();
    const cleared = clearedToday(doc, day);
    const [rank, players] = await Promise.all([rankOf(key), walletCount()]);
    // The tasks the owner published after this file was written, which live in the store. Read once
    // here so the tab, the tab's count and the payout path all work from the same list.
    const extras = await extraOneTimeTasks();
    ...
    const referrals = await Promise.all((doc.referrals || []).map(async (addr) => {
        ...
        return {
            ...
            // Whether this invitee has crossed the line that starts the commission — the same
            // function the payout uses, so the list cannot promise a payment the store will not
            // make. It explains a row that shows `Their PTS` with `You earned 0`.
            counting: referralQualifies(them),
        };
    }));
    ...
    // The invite link travels with the post, so it carries the code — five characters read aloud
    // rather than a 42-character address. `referralUrl` falls back to the address if a wallet has no
    // code yet, which is the one case where promise and post would otherwise disagree.
    const inviteUrl = referralUrl(refCode || key);
    const shareText = sharePostText({ points: sharePoints, refUrl: inviteUrl, day });
    ...
    return {
        ...
        rank,
        players,
        // What this wallet was paid and why, newest first. A wallet that earned before the log
        // existed has none, which is why the page says "from now on" rather than pretending the
        // list is complete — `recentTotal` counts what is actually held, not what was ever earned.
        recent: (doc.activity || []).slice().reverse().slice(0, ACTIVITY_SHOWN),
        recentTotal: (doc.activity || []).length,
        ...
        // The streak, which is what today's third floor pays *on top* of the run. Offered as the
        // whole answer — which day today is, what that day pays, and whether it has been paid — so
        // the page never has to work the ladder out for itself.
        streak: streakFor(doc, day),
        // Which day of the season today is, and the marker a shared post carries for it. Both come
        // from the same helper the verifier is handed, so the post text, the check and the page
        // cannot say three different things about what day it is.
        seasonDay: programDay(day),
        dayMarker: dayMarker(day),
        ...
        referrals,
        // How many of the invited wallets are actually paying: the number a player cares about, and
        // the one the panel counts rather than passing off every join as a referral.
        referralsCounting: referrals.filter((ref) => ref.counting).length,
        referralMinPoints: REFERRAL_MIN_POINTS,
        referrer: doc.referrer || null,
        // `refCode` is what the player types back; the link is the same thing for pasting. Neither is
        // a secret — a code that could not be shared would be no use as an invite.
        refCode,
        inviteUrl,
        referredAt: doc.referredAt || null,
        ...
        // along as the link's card (and is previewed on the card, from the same file), the text
        // itself, and the composer link with it prefilled.
        share: {
            tag: X_SHARE_TAG,
            ogImage: SHARE_OG_IMAGE,
            text: shareText,
            intentUrl: shareIntentUrl(shareText),
            // The marker printed in the text above, for the one line that tells the player to leave
            // it alone — the check looks for this exact string, so the page should not retype it.
            dayMarker: dayMarker(day),
        },
        ...
        oneTime: oneTimeTasks(doc, extras).map((task) => oneTimeView(doc, task)),
        ...
        // Today's board, the last draw, this wallet's capsules and the one waiting to be claimed.
        // Composed from the record already in hand rather than read again, so the panel costs no
        // extra round trip — and it is one function, so what a signed-in page shows and what the
        // public board serves cannot become two different answers.
        giveaway: await giveawayView(key, { doc }),
...
/**
 * Turn whatever arrived in `?ref=` — or was typed into the add-code box — into the wallet it credits.
 *
 * Address first, then code. Both stay supported: links carrying an address are already published in
 * posts, and a link that used to work quietly stops counting the day the code form ships. The code
 * is read case-insensitively, and a whole pasted invite link is unwrapped to the code inside it.
 */
async function resolveRef(input) {
    const token = refTokenFromInput(input);
    if (!token) return null;
    return normaliseAddress(token) || refCodeOwner(token);
}

/** The code this wallet hands out, minting one on the spot if it has never been asked for one. */
async function ensureRefCode(key) {
    const doc = await getWallet(key);
    if (!doc) return null;
    return doc.refCode || assignRefCode(key);
}
...
export async function registerVisit(address, refCode) {
    const key = normaliseAddress(address);
    if (!key) return null;
    const ref = await resolveRef(refCode);
    ...
    // Arriving is when a wallet joins, so it is when it gets a code of its own to hand out.
    await ensureRefCode(key);
...
/**
 * Attach a referrer to a wallet that has none, long after it joined.
 *
 * The path for a player who started by playing and only later met the person whose link they want to
 * credit. Three rules hold it, and only the third is new:
 *
 *   - **once** — a wallet that already has a referrer keeps it. A referrer that could be swapped
 *     would move commissions that have already been paid, and it would hand the same referee to
 *     somebody new every day.
 *   - **never itself** — the obvious loop.
 *   - **never a cycle** — which exists *because* this feature does. When a referral can be recorded
 *     after arrival, A can attach B who attached A, and then `credit`'s second-degree walk
 *     (`referrer.referrer`) pays A a commission out of A's own points. At arrival that shape was
 *     impossible — neither wallet could have a referrer yet — so the guard was never needed before.
 *     `credit` also refuses a self-payment outright, so the invariant is held in two places: here
 *     where the chain is built, and there where the points move.
 *
 * Returns `{ ok: true, state }`, or `{ error, code }` with a code the page switches on.
 */
export async function attachRef(address, input) {
    const key = normaliseAddress(address);
    ...
    if (!token) {
        return {
            error: 'That is not an invite code — five letters and digits, no 0, 1, I, L or O.',
            code: 'bad-code',
        };
    }

    const ref = await resolveRef(token);
    if (!ref) {
        return { error: `No wallet holds the code ${token}. Check it with whoever sent it.`, code: 'unknown-code' };
    }
    if (ref === key) return { error: 'That is your own invite code.', code: 'own-code' };

    const doc = await getWallet(key);
    if (doc?.referrer) {
        return { error: 'This wallet already has a referrer, and that does not change.', code: 'already-referred' };
    }

    // Walk the code owner's own chain. If this wallet is anywhere in it, the code would close a loop.
    let cursor = ref;
    for (let hop = 0; cursor && hop < REF_CHAIN_LIMIT; hop += 1) {
        if (cursor === key) {
            return {
                error: 'That code belongs to somebody you referred — it would pay you for your own points.',
                code: 'cycle',
            };
        }
        cursor = (await getWallet(cursor))?.referrer || null;
    }

    // Referee first, then the referrer's list: a wallet must never be listed as having referred
    // someone it does not actually refer. The re-check inside the write is for a race with a
    // `?ref=` claim landing in the same instant.
    await updateWallet(key, (w) => {
        if (!w.referrer) {
            w.referrer = ref;
            w.referredAt = new Date().toISOString();
        }
        return w;
    });
    await updateWallet(ref, (w) => {
    ...
    return { ok: true, state: await stateFor(key) };
}

/**
 * How many entries a wallet's earning log keeps, and how many of them a page is shown.
 *
 * The log exists so the Portfolio can list what was paid and why instead of showing a total with
 * no story. It is capped because it is per wallet in a shared store: forty small objects is a
 * rounding error beside what the record already holds, and an unbounded list would not be.
 */
const ACTIVITY_LOG_LIMIT = 40;
const ACTIVITY_SHOWN = 12;

/**
 * Append one movement of points to the wallet's log, newest last, oldest dropped.
 *
 * One writer for both directions, because the log is read as a story and a spend that did not appear
 * in it would leave a balance that fell by nothing the player can name. `logEarn` and `logSpend` are
 * the two ways in; the figure is signed, and the Portfolio prints the sign it is given.
 */
function logPoints(w, points, reason) {
    const log = Array.isArray(w.activity) ? w.activity.slice() : [];
    log.push({ at: Date.now(), points, reason });
    w.activity = log.slice(-ACTIVITY_LOG_LIMIT);
    return w;
}

/** Append one earning — a vault floor, a streak, a commission, a task. */
function logEarn(w, points, reason) {
    return logPoints(w, points, reason);
}

/** Append one spend: the same row, negative. */
function logSpend(w, points, reason) {
    return logPoints(w, -Math.abs(Math.floor(points)), reason);
}
 ...
 *
 * Every payment goes through here, which is why the log is written here and not at the call sites:
 * a row in the Portfolio is then the same event the balance was summed from, and a new way to earn
 * cannot be added without appearing in the list.
 *
 * **The day's board is written here too**, for the same reason: an earning is the only event that can
 * change it, and the daily capsule draw ranks it. One write, to one place — the day's own board,
 * which carries both the points and the stamp the draw tie-breaks on — because a tally kept on the
 * wallet beside it would be a second answer to "what did this player earn today", and two answers
 * are one more than can be kept in step.
 *
 * It is skipped for a commission: `countsTowardDaily` is the single place that decides what belongs
 * on a board that hands out prizes, and a referral's cut is paid for somebody else's play rather than
 * for today's.
 */
async function credit(address, amount, reason) {
    ...
    const doc = await updateWallet(address, (w) => {
        ...
        return logEarn(w, value, reason);
    });

    if (countsTowardDaily(reason)) await bumpDailyPoints(address, todayKey(), value, Date.now());

    // ------------------------------------------------- is this invitee a real player yet?
    // The gate sits *here*, above the first payment, so there is one place it can be got wrong
    // rather than one per tier. An invited wallet earns for its inviter only once it has bound an X
    // account and has ten points of its own (`referralQualifies`, which the page reads too). Both
    // tiers below are downstream of this return, so neither can pay on an invite that is not real.
    //
    // No back-pay, deliberately: the credits that arrived before the wallet qualified are not
    // revisited, so every payment is one event in the referrer's own log and the ledger can be read
    // back to the earning that caused it.
    if (!referralQualifies(doc)) return value;

    // Commissions: 15% to whoever referred this wallet, 5% to whoever referred them. A wallet is
    // never paid out of its own earning — `attachRef` refuses to build a chain that would do it, and
    // this refuses to pay one, so the path that moves points is safe on its own terms rather than on
    // the other guard's care.
    const referrer = doc?.referrer ? await getWallet(doc.referrer) : null;
    if (!referrer || referrer.address === address) return value;
    ...
    if (first) {
        await updateWallet(referrer.address, (w) => {
            ...
            // Commission is points this wallet earned, so it belongs in its own log — the Portfolio
            // would otherwise show a balance that grew by nothing it can name.
            return logEarn(w, first, 'referral_commission');
        });
    }

    const secondAddress = referrer.referrer;
    const fifth = commission(value, REFERRAL_2ND_PCT);
    if (secondAddress && secondAddress !== address && secondAddress !== referrer.address && fifth) {
        ...
        if (second) {
            await updateWallet(second.address, (w) => {
                ...
                return logEarn(w, fifth, 'referral_commission');
            });
        }
    }
    return value;
...
/**
 * Spend points on something the player chose to buy, and write it down.
 *
 * The mirror of `credit`, and deliberately not its opposite number in every respect: a spend pays
 * nobody a commission, and it **does not touch the day's board** (`bumpDailyPoints` is not called
 * here). The daily board is what the capsule draw ranks — points earned by playing today — and a
 * player who spends a thousand of them has still played today. Letting a spend rewrite that board
 * would quietly cost them their place in the draw for the points they just redeemed.
 *
 * The balance is checked **inside the write**, not before it, so the write itself cannot overdraw a
 * wallet however the caller is called. `updateWallet` is a read-modify-write with no transaction
 * under it, so the caller still has to hold the wallet's own guard for the *sequence* — which `spin`
 * in `lib/points-redeem.js` does (`claimGuard('redeem:spin:<address>')`).
 *
 * `extra` is applied inside the same write, which is what lets a redeem record the code it just
 * handed over in one step rather than two writes that can disagree. Returns
 * `{ ok, spent, points, doc }`, or `{ ok: false, error: 'not_enough_points', points }`.
 */
export async function spendPoints(address, amount, reason = 'redeem_spin', extra = null) {
    const key = normaliseAddress(address);
    const value = Math.floor(Number(amount) || 0);
    if (!key || value <= 0) return { ok: false, error: 'bad_amount' };

    let short = null;
    const doc = await updateWallet(key, (w) => {
        const have = Math.max(0, Math.floor(Number(w.points) || 0));
        if (have < value) {
            short = have;
            return w;
        }
        ...
        return logSpend(w, value, reason);
    });

    if (short !== null) {
        return { ok: false, error: 'not_enough_points', points: short, needed: value };
    }
    return { ok: true, spent: value, points: doc?.points ?? null, doc };
}
...
export async function clearLevel(address, level) {
    ...
    // ---------------------------------------------------------------- the streak bonus
    // The third floor is the moment the streak is earned, and it is earned once a day like every
    // other award here: read the record, claim the day, write it down, *then* pay. Two simultaneous
    // clears of the last floor both reach this line, and the guard is what makes exactly one of them
    // pay — which is also why the record is written before the credit rather than after.
    let streakAward = 0;
    if (cleared.length + 1 >= VAULT_LEVELS.length) {
        streakAward = await settleStreak(key, day);
    }

    return { credited, streakAward, state: await stateFor(key) };
}

/**
 * Pay today's streak bonus, if today's run is what just finished.
 *
 * Split out of `clearLevel` so the rule — one bonus a day, on the day's last floor, recorded
 * before it is paid — is in one readable place. Returns what was actually credited, which is 0
 * for a day already paid, a wallet whose bonus was already recorded, or a credit that failed
 * after the record was written (that last case is logged with the numbers, and is deliberately
 * **not** retried on the next clear: the day is spent, and the log is the recovery path).
 */
async function settleStreak(key, day) {
    const before = (await getWallet(key)) || blankWallet(key);
    const streak = streakFor(before, day);
    if (streak.paid) return 0;
    if (!(await claimGuard(`streak:${key}:${day}`))) return 0;

    await updateWallet(key, (w) => {
        w.streak = { days: streak.days, lastDay: day, award: streak.award };
        return w;
    });

    try {
        return await credit(key, streak.award, `streak_day_${streak.days}`);
    } catch (error) {
        console.error(`[points] streak day ${streak.days} recorded but not credited for ${key} on ${day}: ${streak.award} PTS — ${error.message || error}`);
        return 0;
    }
}
...
async function settle(key, config, url, record = null) {
    ...
    // Scoped to the day for a daily task. The guard is what stops two taps arriving together from
    // paying twice, and for a daily reward "twice" can only mean twice **today** — without the day
    // in the key the guard would be answering about a different day's payment.
    const creditGuard = config.daily
        ? `task-credit:${key}:${config.taskId}:${config.day}`
        : `task-credit:${key}:${config.taskId}`;
    if (!(await claimGuard(creditGuard))) {
    ...
    await updateWallet(key, (w) => {
        ...
        // Additive: this map is a calendar, and the day a run was doubled is worth keeping beside
        // the days before it rather than replacing them.
        if (config.kind === 'share') w.sharedOn = { ...(w.sharedOn || {}), [config.day]: true };
        return w;
    });
...
export async function submitTask(address, kind, url) {
    ...
    const config = taskConfig(kind, doc, todayKey(), await extraOneTimeTasks());
    if (config.error) return config;

    // A **daily** task lives in one record slot — `share` — and the record carries the day it was
    // verified on. So a verified record from an *earlier* day answers about yesterday, not about
    // today, and the daily bonus stops being daily: the second day's submission is refused as
    // `alreadyCredited`, and the player is told "that one is already paid" about a post they made
    // minutes ago. `taskView` already drops a record from another day, which is why the page kept
    // offering the button while the payout refused it. This makes the gate agree with the view.
    const record = doc.tasks?.[config.taskId] || null;
    const fromAnotherDay = config.daily && record?.day && record.day !== config.day;
    if (!fromAnotherDay && record?.state === 'verified' && (record.credited || 0) > 0) {
        return { credited: 0, alreadyCredited: true, state: await stateFor(key) };
    }
...
export async function checkTask(address, kind) {
    ...
    // player is told which day it was about.
    //
    // This is asked **before** the two state answers below, because for a record from an earlier day
    // "yesterday" is the true answer: it is not pending, and a share that was paid on an earlier day
    // is not something today's check may call `alreadyCredited`. A record that was already **paid**
    // is left exactly as it is — the receipt says what was true on the day, and rewriting it as
    // `expired` would turn a settled claim into a failure.
...
export async function claimOneTime(address, taskId) {
    ...
    //
    // Only the follow asks this question. A `visit` task (the waitlist) has no mode to look up and
    // no fact to wait for, so it falls straight through to the record below, which is why it pays in
    // the same call that claims it.
 ...
 *
 * The board names a player by **the account they earn from**, which is the X handle they bound,
 * falling back to the address only for a wallet that has bound nothing — because an address is what
 * a wallet has when it has no name yet, and a name is what everybody else on the board goes by. A
 * handle is also a name somebody **typed**, so `handleProved` comes back beside it: the page shows a
 * check only where Privy vouched for the account, which is the one thing separating a player's own
 * name from somebody else's typed into a box. Nothing here is a lookup — the address stays in the
 * row, because it is what the board client already keys on and what the API has always published.
 *
 * The default is the page's board size (`BOARD_LIMIT`), not a larger one: a length no caller asked
 * for is a length somebody has to keep in step, and the two would only drift.
 */
export async function leaderboard(limit = 10, me = null) {
    const wallets = await topWallets(limit);
    const mine = normaliseAddress(me);
    return wallets.map((w, i) => {
        const handle = w.x?.username || null;
        return {
            rank: i + 1,
            address: w.address,
            short: short(w.address),
            handle,
            handleProved: !!handle && !!w.x?.verified,
            points: w.points,
