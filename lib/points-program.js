...
export async function shareEntry(address) {
    ...
    // Same guard as the floors: the share bonus is once a day, and two taps on the
    // button must not pay it twice.
    if (!(await claimGuard(`share:${key}:${day}`))) {
        return { credited: 0, alreadyShared: true, state: await stateFor(key) };
    }
    ...
    let credited = 0;
    try {
        credited = await credit(key, earned, 'vault_share_bonus');
    } catch (error) {
        console.error(`[points] share bonus recorded but not credited for ${key} on ${day}: ${earned} PTS — ${error.message || error}`);
    }
