...
import {
    ...
    updateWallet, walletCount, claimXBinding, releaseXBinding, xBindingOwner,
 ...
 * and the post is wrong: the wrong author, or missing our link) and `expired` (it was never
 * visible to X within the attempt ceiling, or the day it belonged to ended first). "We could not
 ...
 *
 * Both tasks are proved by the same module with the same two expectations — the **bound handle**
 * wrote it, and it **carries our link** — which is the point: one verifier, two rewards, and no
 * second place for the rules to drift to. What differs is only the amount and how often it pays.
 */
function taskConfig(kind, doc, day = todayKey()) {
    ...
    if (kind === 'share') {
        ...
        return {
            ...
            amount: earned,
            expect: { handle, host: SITE_LINK_HOST },
        };
    }
...
export async function bindX(address, identity = {}, { verified = false } = {}) {
    ...
    if (!claim.ok) {
        ...
        const canTakeOver = verified && owner?.x?.verified !== true;
        if (!canTakeOver) {
            return {
                error: `@${username} is already bound to another wallet (${short(claim.owner)}). One X account earns for one wallet.`,
                code: 'x-taken',
    ...
    // One person can hold two index entries: `handle:<name>` from a provisional binding and the
    // numeric id from a proved one. When the proof arrives, the name's provisional claim is handed
    // over — but only if it is still provisional, so a proved binding is never displaced by a later
    // handle-only one.
    if (verified && id !== `handle:${username}`) {
        const holder = await xBindingOwner(`handle:${username}`);
        if (holder && holder !== key) {
            const theirs = await getWallet(holder);
            if (theirs?.x?.id === `handle:${username}` && theirs.x.verified !== true) {
                await releaseXBinding(`handle:${username}`, holder, { force: true });
                await updateWallet(holder, (w) => {
                    if (w.x?.id === `handle:${username}` && w.x.verified !== true) w.x = null;
                    return w;
                });
            }
        }
    ...
    const dropped = Object.entries(doc.tasks || {})
        ...
        .map(([taskId]) => taskId);
    const previousId = held?.id || null;

    await updateWallet(key, (w) => {
        w.x = { id, username, verified: verified === true, boundAt: new Date().toISOString() };
        for (const taskId of dropped) if (w.tasks && w.tasks[taskId]) delete w.tasks[taskId];
        return w;
    });
    if (previousId && previousId !== id) await releaseXBinding(previousId, key);
...
/** Hand the X account back. Points already earned stay — this switches off *future* earning. */
export async function unbindX(address) {
    const key = normaliseAddress(address);
    if (!key) return { error: 'invalid address' };
    const doc = await getWallet(key);
    const xid = doc?.x?.id || null;
    await updateWallet(key, (w) => {
        w.x = null;
        return w;
    });
    if (xid) await releaseXBinding(xid, key);
 ...
 * often; the proof is the same module with the same two expectations — the **bound handle** wrote
 * the post, and it **carries our site link**. Nothing here trusts the client for anything except
 * the URL of a post the player says is theirs.
 */
 ...
 * from X (someone else's post, no link, not a post at all) and never pays, no matter how often it
 * is retried. `expired` is "no good answer inside the attempt ceiling", which also never pays.
 */
