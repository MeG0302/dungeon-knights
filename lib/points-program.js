 ...
 * nothing is checked. That is the honest shape for this tab, and the copy says so — a card that
 * rendered a spinner for a check that never runs would be a lie about the rules.
 *
 ...
 * it does not call the verifier, and that is a decision rather than an omission. The one task in
 * the tab is a follow, and no free X endpoint can see who a player follows — `lib/x-verify.js`
 * explains at length what oEmbed can and cannot prove. Inventing a check here would be worse than
 * having none: a check that always passes teaches a player that the rules are decoration.
 *
 * So the honest shape is: claim it, and the page says plainly that it is taken on their word.
 * What is *not* on their word is the account — the X binding is required, and a binding cannot be
 * moved to a second wallet, which is what keeps one account from claiming the same task twice.
 */
export async function claimOneTime(address, taskId) {
    ...
    if (doc.tasks?.[recordId]?.state === 'verified') {
        return { credited: 0, alreadyCredited: true, state: await stateFor(key) };
    }

    // The same claim the floors and the X tasks take, for the same reason: the read above and the
    // write below are two steps, so two taps arriving together would otherwise both pay.
    ...
    // line below, a second payment is points out of a fixed pool.
    await updateWallet(key, (w) => {
        w.tasks = { ...(w.tasks || {}) };
        w.tasks[recordId] = {
            state: 'verified',
