 ...
 * one: the tab should be about what is still worth doing, not a wall of what is not. The follow is
 * the exception — it stays, marked claimed, because it is the one task every player has, and a card
 * that disappears for some players and not others reads as a bug rather than as a receipt.
 *
...
function oneTimeTasks(doc, extras = []) {
    return [...ONE_TIME_TASKS, ...extras].filter((task) => {
        if (task.proof === 'claim') return true;
