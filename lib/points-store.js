...
export function blankWallet(address) {
    return {
        ...
        capsuleNotice: null,
        // What this wallet has spent points on, newest last: `{ id, at, outcome, prize, code, cost }`.
        // A spin that lost is a row too, with `outcome: 'tryAgain'` and no code — which is what makes
        // a lost spin as readable as a won one when a player asks what happened to their points.
        // The `id` is the browser's own spin id, so a request that arrives twice is answered from
        // this list instead of being charged twice; see `lib/points-redeem.js`.
        redeems: [],
