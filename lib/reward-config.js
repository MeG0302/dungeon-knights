...
import { CAPSULES_PER_WEEK, KNIGHTS_CAP } from './staking-config.js';
...
export const REFERENCE_UTILISATION = {
    ...
    knights: REFERENCE_KNIGHTS_ACTIVE / KNIGHTS_CAP,
};
...
/** Every summable Knight at the collection cap, playing and staking. */
export function knightsMaximumBurnPerDay(cap = KNIGHTS_CAP) {
    const dungeon = cap * expectedRewardPerDay();
    return dungeon * (1 + STAKING_SHARE_OF_DUNGEON);
}

/** Absolute maximum daily burn — the number that makes the worst case computable. */
export function worstCaseBurnPerDay() {
    return genesisMaximumBurnPerDay() + knightsMaximumBurnPerDay();
}

/**
 * What one Knight earns per day once the collection is full, at the reference budget.
 *
 * Both Knights lines are fixed shares of the budget, so filling the collection divides them
 * among twenty times as many knights: at the cap a Knight earns exactly
 * `REFERENCE_KNIGHTS_ACTIVE / KNIGHTS_CAP` of what it earns at the reference — **5.0%**.
 *
 ...
 */
export function knightsPayoutAtCap(cap = KNIGHTS_CAP) {
    const lines = referenceLines();
    const dungeonPerKnight = lines.knightsDungeon / cap;
    const stakingPerKnight = lines.knightsStaking / cap;
...
// ------------------------------------------------------------- the caps and prices
/** Capsules mint Knights, so the collection's growth is a faucet and needs a ceiling. */
export { KNIGHTS_CAP };
 ...
 * 500 -> 5,000 across the cap, and the crossover is worth knowing because it is the point
 * from which the faucet funds itself: **from roughly 5,700 Knights on, the 200 weekly opens
 * alone cover the whole Knights lines.**
 */
export function capsuleOpenPrice(minted, cap = KNIGHTS_CAP) {
    const base = 500;
    const atCap = 5000;
    const filled = Math.min(1, Math.max(0, (Number(minted) || 0) / cap));
    return base + (atCap - base) * filled;
}
...
export function capsuleBreakEvenMinted() {
    ...
    const span = 5000 - 500;
    return Math.round(((priceNeeded - 500) / span) * KNIGHTS_CAP);
}
...
export function economy() {
    ...
    return {
        ...
        knightsPayoutAtCap: knightsPayoutAtCap(),
