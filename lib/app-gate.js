...
/**
 * Which hostnames are behind the gate. Comma-separated so a preview hostname can be added without
 * touching code — and so the list is **explicit**: everything not named here (the apex,
 * `*.vercel.app`, the webhook's own host) keeps behaving exactly as it did. An allowlist rather than
 * "everything except the apex", because the X webhook is registered against
 * `dungeon-knights.vercel.app`, and a gate that swept that up would silently break a paid
 * registration.
 */
 ...
 *
 * There are three classes, and every host falls in exactly one:
 *
 ...
 *   **anything else**  *untouched*. A preview URL, the vercel hostname, a localhost port: they serve
 *                      what they served before this feature existed. Nothing about this change is
 ...
 *
 * Two ways to be: on a gated hostname, or — in development only — a cookie says so. The cookie
 * exists because `localhost:3000` cannot be two hostnames, so the whole split would otherwise be
 * untestable before it is published. It is ignored in production.
 */
