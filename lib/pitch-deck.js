 ...
 * a list of blocks (`kicker`, `title`, `lede`, `chips`, `stats`, `cards`, `steps`, `flow`, `table`,
 * `quote`, `live`, `note`, `links`), and adding a slide never means touching the renderer. `flow` is
 * the one diagram: the loop a player walks, as a row of numbered boxes with the stylesheet's arrows
 * between them — boxes rather than an SVG, so it re-wraps on a phone instead of shrinking to
 * illegibility.
 *
...
/**
 * `12:00 UTC` — the boundary the daily run caps roll over on.
 *
 * Written once, in the shape the contracts name it: their `currentDayIndex()` is the hour in
 * `GAME_RESET_HOUR_UTC`, so the deck prints the same instant a `runsUsed` counter resets on rather
 * than a clock it invented. `tools/check-run-budget.js` reads this constant, the browser's copy in
 * `public/run-budget.js` and both contracts, and fails if any of them disagrees.
 */
const resetClock = `${String(GAME_RESET_HOUR_UTC).padStart(2, '0')}:00 UTC`;

/** The per-knight daily cap of every tier, Common first — `5 / 5 / 4 / 3 / 4`. */
const runCapsLadder = KNIGHT_TIERS.map((key) => RARITY[key].dailyRuns).join(' / ');
...
export const SLIDES = [
    ...
    {
        id: 'day',
        nav: 'A day at the gate',
        blocks: [
            { kind: 'kicker', text: '04 · A day at the gate' },
            { kind: 'title', text: 'The loop a player actually walks, and the five runs it stops at.' },
            {
                kind: 'lede',
                text: 'Five screens in the order they are met, and two of them exist only to *stop* a run: the loading gate that holds a map until its art has settled, and the run budget that holds a squad once it is spent. Where the limit is finally enforced is not in the browser at all.',
            },
            {
                kind: 'flow',
                nodes: [
                    {
                        step: '01',
                        title: 'Knights Hall',
                        text: 'Up to **15 knights** are picked. Each card prints the runs *that knight* has left today, read from the chain rather than assumed.',
                    },
                    {
                        step: '02',
                        title: 'Quest board',
                        text: 'One of five themed maps, then a loading transition covers the hand-off into the dungeon.',
                    },
                    {
                        step: '03',
                        title: 'The gate',
                        text: 'Floor, monsters and sprites arrive behind a screen that lifts once they have settled — the squad runs *then*, not behind it.',
                    },
                    {
                        step: '04',
                        title: 'The run',
                        text: 'The engine fights it out and the loot is cleared. Every knight in the squad is one run, and none of them is spent yet.',
                    },
                    {
                        step: '05',
                        title: 'Claim',
                        text: '`claimSignedRuns` charges one run to each knight and requires `runsUsed < dailyCap[rarity]`. **The sixth is refused here, on chain**, whoever asks.',
                    },
                ],
                caption: `Then the auto-progress rebuilds the dungeon behind the same gate, and the next run opens as it lifts — until the budget is spent, when the squad rests with a sentence instead of clearing a dungeon nobody can be paid for. Every cap refreshes at **${resetClock}** for every wallet at once: \`currentDayIndex()\` in both game contracts.`,
            },
            {
                kind: 'stats',
                items: [
                    {
                        value: runCapsLadder,
                        label: 'Runs a day, Common → Legendary',
                        sub: "Per knight, and the contract's own `dailyCap` — the client only reads it.",
                    },
                    {
                        value: fmt(GENESIS_DAILY_RUNS),
                        label: "A Genesis Knight's own cap",
                        sub: 'A separate counter. The two collections are never pooled.',
                    },
                    {
                        value: resetClock,
                        label: 'The reset boundary',
                        sub: 'A lunchtime rollover, an hour the contracts name themselves.',
                    },
                ],
            },
        ],
    ...
    /* ---------------------------------------------------------------- 6 · trust */
    {
        ...
        nav: 'The gate',
        blocks: [
            { kind: 'kicker', text: '05 · The trust layer' },
    ...
    /* -------------------------------------------------------------- 6 · economy */
    {
        ...
        nav: 'The economy',
        blocks: [
            { kind: 'kicker', text: '06 · The economy' },
    ...
    /* ---------------------------------------------------------- 7 · unit economics */
    {
        ...
        nav: 'Unit economics',
        blocks: [
            { kind: 'kicker', text: '07 · Unit economics' },
    ...
    /* ------------------------------------------------- 8 · points + daily capsule */
    {
        ...
        blocks: [
            { kind: 'kicker', text: '08 · Growth' },
            { kind: 'title', text: 'The Points Program, and a Knight a day for ten players.' },
            {
                kind: 'lede',
    ...
    /* ----------------------------------------------------------- 11 · disclosure */
    {
        ...
        nav: 'Disclosures',
        blocks: [
            { kind: 'kicker', text: '11 · Disclosures' },
