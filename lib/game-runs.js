...
export const ADDRESSES = {
    // The **third** collection, and the two fallbacks before it were both wrong in ways worth
    // remembering: the first (`0x06c7…`) was the original collection, and the second
    // (`0xFB738…`) was deployed but could not be minted into, because `summon()` never pulled the
    // player's 500 DNG. This one is the repaired collection — see `docs/DEPLOY-PHASE-2.md`. The
    // fallback is only used when the env is missing, which is exactly when a stale address does
    // the most damage: every route would read a collection nothing else in the game uses.
    knightNFT: process.env.KNIGHT_NFT_ADDRESS || '0x27Cfbb763188a50Fe1C0fFfBe2552b1945eE1B2D',
    // The fixed-supply collection. Unlike the summonable one it can be deployed and still hold
    // nothing — every Genesis knight is minted deliberately — so "configured" and "has knights"
    // are different facts and both routes need the address rather than a guess about it.
    genesisNFT: (process.env.GENESIS_NFT || '').trim(),
    // Phase 2 $DNG. The first token (`0xA8D5…`) is not what any contract in this game pays in any
    // more, so it must not be what an unconfigured server reads either.
    dngToken: process.env.DNG_TOKEN_ADDRESS || '0x3D94e56E0d967633830f6d9E42CE43A64FFfD6Ca',
    gameV3: process.env.GAME_CONTRACT_V3 || '0xD8de9385Db7DfE925882E76849B6e067e47236e5',
    gameV4: (process.env.GAME_CONTRACT_V4 || '').trim(),
    // One staking pool per collection, and they are not interchangeable: Genesis stakes for pool
    // yield *and* raffle tickets, the summonable Knights for yield alone. Resolved here rather than
    // in each route because the vault now both reads and writes them: two resolutions is how the
    // page ends up reading one pool and staking into another. `STAKING_CONTRACT` survives as the
    // Genesis side's fallback for a deployment that only ever had one.
    genesisStaking: (process.env.GENESIS_STAKING || process.env.STAKING_CONTRACT || '').trim(),
    knightsStaking: (process.env.KNIGHTS_STAKING || '').trim(),
};
