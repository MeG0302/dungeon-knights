...
function emptySnapshot(nowMs, poolDng = null, collection = 'genesis') {
    ...
    return {
        ...
        // the empty shape; `chainSnapshot` is the only place that can answer it.
        simulated: false,
...
export function chainSnapshot(address, config, nowMs = Date.now(), collection = 'genesis', holdings = null) {
    ...
    // Real knights, no staking contract. The vault runs as a simulation rather than going inert.
    const simulated = sideLive && !stakingDeployed;
    ...
    return withShares({
        ...
        // chain is what `simulated` answers.
        canWrite: sideLive,
        simulated,
