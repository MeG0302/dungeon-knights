    ...
    return {
        ...
        staked: staked.map((knight) => {
            const accruedShare = accruedPoolShare({
                ...
                // A claimed stake accrues from the claim, not from the stake. Without this the
                // DNG a player just took stayed on the card and could be taken again.
                settledAt: knight.claimedAt || null,
            }, nowMs);
            return {
                ...knight,
...
export async function loadVault(
    address, { config, nowMs = Date.now(), fetchJson, collection = 'genesis', fresh = false } = {},
) {
...
export function applyAction(snapshot, action, payload = {}, nowMs = Date.now()) {
    ...
    switch (action) {
        ...
        case 'claim': {
            ...
            // The claim settles this stake: `claimedAt` is what `withShares` reads to restart the
            // accrual, and it is stamped with the same instant the amount was measured at, so the
            // DNG paid and the DNG removed are the same number.
            next.staked = next.staked.map((k) => (k.tokenId === knight.tokenId
                ? { ...k, claimed: true, claimedAt: nowMs }
                : k));
            return {
                snapshot: withShares(next, nowMs),
