...
// One constant, and the middleware is allowed to read it: `points-config` imports nothing but the
// site's own constants, so pulling it in here does not put a driver or a fetch on the edge path.
import { REDEEM_LIVE } from './points-config.js';
 ...
 *   `/api/points/draw`           the daily capsule draw, called by Vercel's cron. A scheduler
 *                                cannot type a password either, and the same argument decides it: the
 *                                route authenticates itself — `Authorization: Bearer $CRON_SECRET`,
 *                                compared in constant time — and **refuses with 503 when that secret
 *                                is not configured**, so the exemption cannot outlive the check. The
 *                                draw does not depend on this path being reachable at all: page reads
 *                                settle it too, which is why failing closed here costs nothing.
 */
export const GLOBAL_OPEN = ['/api/x/events', '/api/discord/interactions', '/api/points/draw'];
...
export const APEX_PUBLIC = [
    ...
    '/genesis',
    // The documentation. Public for the same reason as the three above it — it is written *for*
    // somebody who has not arrived yet, and a document behind a password is a document nobody
    // reads. It reads nothing and writes nothing: `/docs` is content, the two files it stands on are
    // under `lib/`, and the figures in them come from the same modules the contracts were deployed
    // from. There is nothing here to withhold.
    '/docs',
    ...
    '/portfolio',
    // Redeeming points, and here for the same reason as the two above it: it is the page a *winner*
    // is sent to. A gift card behind a password is a gift card that needs the game's front door
    // opened first, which is the opposite of what a prize should cost — and the page is useless
    // without a signed wallet, so being public gives away nothing but its own existence.
    //
    // Conditional because the wheel is **held back** (`REDEEM_LIVE`): until cards are loaded there
    // is nothing to win, and a public wheel that cannot pay is worse than a wheel nobody can reach.
    // While it is off, `/redeem` falls through to `other` below and is answered exactly like the
    // rest of the unpublished game — reachable on the game's host, behind the password.
    ...(REDEEM_LIVE ? ['/redeem'] : []),
    '/api/points',
    '/api/waitlist',
...
/**
 * Paths a published feature has *not* published yet, held back as a whole.
 *
 * A list of its own because the prefix entries above are deliberately coarse: `/api/points` is open
 * on the apex so the whole program works without a password, and it therefore also swallows
 * `/api/points/redeem` — a wheel endpoint that spends a balance and pops a real gift code. While the
 * wheel is held back that endpoint is held back with it, and this is checked *before* the prefixes
 * rather than after, because "open except" cannot be expressed from the other side.
 */
const HELD_BACK = REDEEM_LIVE ? [] : ['/api/points/redeem'];
...
export function classify(pathname) {
    ...
    if (startsWithAny(pathname, HELD_BACK)) return 'other';
