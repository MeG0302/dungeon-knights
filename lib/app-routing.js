...
/** Signature-authenticated, so it stays reachable on any host — the X webhook lives here. */
export const GLOBAL_OPEN = ['/api/x/events'];
...
export const APEX_PUBLIC = [
    ...
    '/api/points',
    '/api/waitlist',
    // The portfolio page is app-only, but the Privy login on the apex points page reads the chain
    // config through here. Reads only — no keys, no writes.
    '/api/wallet',
];
