...
const NFT_IFACE = new utils.Interface([
    ...
    // The second shape. `GenesisKnights` has 1,024 knights and no tiers — what it publishes is
    // the hash power it was rolled with — so it answers a different question, and it does not have
    // `getKnightInfo` at all. Both are in this one interface because both are read by the same
    // function below, which asks the collection which of them it answers rather than trusting the
    // address it was handed.
    'function getGenesisKnightInfo(uint256 tokenId) view returns (address owner, uint16 hashPower, uint8 bandIndex)',
    'function balanceOf(address owner) view returns (uint256)',
    'function tokenOfOwnerByIndex(address owner, uint256 index) view returns (uint256)',
    'function supportsInterface(bytes4 interfaceId) view returns (bool)',
    'function name() view returns (string)',
    // The Genesis collection describes its own supply: six bands, and how many rolls are left in
    // each. Reading it here rather than from `lib/staking-config.js` is the same rule as everywhere
    // else in this file — the contract is asked, and the published table is what the answer is
    // checked against.
    'function bands() view returns ((string name, uint16 lo, uint16 hi, uint16 count)[6])',
    'function bandRemaining() view returns (uint16[6])',
]);

/**
 * How much of the fixed 1,024 supply is gone, from the collection's own two views.
 *
 * `bands()` carries the published table (name, range, count) and `bandRemaining()` carries what is
 * left, so minted is a subtraction rather than a number anyone has to keep.
 *
 * `ok: false` with the reason — never a zero — when either read fails: "0 of 1,024 minted" is a
 * claim about the collection, and a failed read is not evidence for it. The page says the supply
 * could not be read instead, which is the true thing.
 */
export async function readGenesisSupply(nftAddress) {
    if (!nftAddress) return { ok: false, reason: 'no Genesis collection is configured for this network' };

    let rows;
    try {
        rows = await rpcBatch([
            { method: 'eth_call', params: [{ to: nftAddress, data: NFT_IFACE.encodeFunctionData('bands', []) }, 'latest'] },
            { method: 'eth_call', params: [{ to: nftAddress, data: NFT_IFACE.encodeFunctionData('bandRemaining', []) }, 'latest'] },
        ]);
    } catch (error) {
        console.warn(`[staking-chain] the Genesis supply could not be read: ${error.message || error}`);
        return { ok: false, reason: 'the chain could not be read just now' };
    }

    if (!rows?.[0] || rows[0].error || !rows?.[1] || rows[1].error) {
        return { ok: false, reason: 'the collection does not publish its bands' };
    }

    try {
        const [table] = NFT_IFACE.decodeFunctionResult('bands', rows[0].result);
        const [remaining] = NFT_IFACE.decodeFunctionResult('bandRemaining', rows[1].result);

        const bands = table.map((band, index) => ({
            name: String(band.name),
            lo: Number(band.lo),
            hi: Number(band.hi),
            count: Number(band.count),
            remaining: Number(remaining[index]),
        }));

        const max = bands.reduce((sum, band) => sum + band.count, 0);
        const left = bands.reduce((sum, band) => sum + band.remaining, 0);

        return { ok: true, max, minted: max - left, remaining: left, bands };
    } catch (error) {
        console.warn(`[staking-chain] the Genesis bands did not decode: ${error.message || error}`);
        return { ok: false, reason: 'the collection answered something this reader cannot read' };
    }
}

/**
 * Which "what is this knight" question a collection answers, decided by asking it.
 *
 * This exists because getting it wrong does not fail: the wrong function reverts, every token is
 * skipped by the `row.error` guard in the loop, and the wallet's own knights come back as a
 * complete-looking empty list with the contract still reporting a non-zero balance — a read that
 * says "you own nothing" about a wallet that owns something. It stayed hidden because Genesis has
 * never been minted, so every Genesis read took the `balance === 0` exit before reaching the
 * question: the fault was one mint away.
 *
 * Probed rather than derived from the address passed in, for the same reason `enumerable` is
 * probed: a constant is a claim about an address this function has never met.
 *
 * Bounded to a few candidates because a burned id reverts on both shapes — that is not evidence
 * about the collection, so the next candidate is asked instead.
 */
async function infoShapeFor(candidates, nftAddress) {
    for (const tokenId of candidates.slice(0, 5)) {
        const rows = await rpcBatch([
            {
                method: 'eth_call',
                params: [{ to: nftAddress, data: NFT_IFACE.encodeFunctionData('getKnightInfo', [tokenId]) }, 'latest'],
            },
            {
                method: 'eth_call',
                params: [{ to: nftAddress, data: NFT_IFACE.encodeFunctionData('getGenesisKnightInfo', [tokenId]) }, 'latest'],
            },
        ]);
        if (!rows[0]?.error) return 'knight';
        if (!rows[1]?.error) return 'genesis';
    }
    // Neither shape answered for a token the collection itself listed. That is a fault in this
    // read, not a wallet holding nothing, and the caller is told so rather than given an empty list.
    return null;
}
...
/**
 * The part of $DNG a wallet page needs: what it holds, and what the token calls itself.
 *
 * `symbol` and `decimals` are read rather than assumed. Eighteen is right here, but it is a
 * fact the token publishes — printing a balance scaled by a decimal count nobody asked the
 * token for is how a reader quietly reports a wrong number by a factor of a billion.
 */
...
/**
 * A wallet's own $DNG balance — the Portfolio page's headline figure.
 *
 * Read **server-side**, through the same `rpcBatch` every other read on this page already uses,
 * and that is the point rather than an implementation detail. The first version of the portfolio
 * asked the browser's wallet provider for this one number, and the result was a page whose other
 * four sections read real chain data while its headline said "the wallet is not available in this
 * browser" — because a saved address and a live extension are two different facts, and the one
 * case where a visitor has the first without the second is exactly the case a portfolio is for.
 * An extension-less browser can read its own holdings from the server; it cannot `eth_call`.
 *
 * Takes the token address from its caller so the route can pass the configured one, and reports
 * a chain failure as `ok: false` with a sentence, like every other read in this file.
 */
export async function readDngBalance(address, { token = null } = {}) {
    const holder = String(address || '').trim().toLowerCase();
    if (!/^0x[0-9a-f]{40}$/.test(holder)) {
        return { ok: false, reason: 'that is not a wallet address' };
    }
    const dngToken = token || ADDRESSES.dngToken;
    if (!dngToken) {
        return { ok: false, reason: 'no $DNG token is configured for this network' };
    }

    let rows;
    try {
        rows = await rpcBatch([
            { method: 'eth_call', params: [{ to: dngToken, data: ERC20_IFACE.encodeFunctionData('balanceOf', [holder]) }, 'latest'] },
            { method: 'eth_call', params: [{ to: dngToken, data: ERC20_IFACE.encodeFunctionData('symbol', []) }, 'latest'] },
            { method: 'eth_call', params: [{ to: dngToken, data: ERC20_IFACE.encodeFunctionData('decimals', []) }, 'latest'] },
        ]);
    } catch (error) {
        console.warn(`[staking-chain] the $DNG balance could not be read: ${error.message || error}`);
        return { ok: false, reason: 'the chain could not be read just now' };
    }

    // The balance is the one call that has to work: without it there is no number to print, and
    // "0 DNG" would be a claim about a wallet nobody managed to ask about.
    if (!rows?.[0] || rows[0].error) {
        return { ok: false, reason: 'the token could not be read just now' };
    }

    try {
        const [raw] = ERC20_IFACE.decodeFunctionResult('balanceOf', rows[0].result);
        // Symbol and decimals are cosmetic by comparison, so a refusal falls back to what this
        // token is known to be rather than failing the read over a label.
        let symbol = 'DNG';
        let decimals = 18;
        if (rows[1] && !rows[1].error) {
            try { [symbol] = ERC20_IFACE.decodeFunctionResult('symbol', rows[1].result); } catch { /* keep DNG */ }
        }
        if (rows[2] && !rows[2].error) {
            try { [decimals] = ERC20_IFACE.decodeFunctionResult('decimals', rows[2].result); } catch { /* keep 18 */ }
        }

        const scale = Number(decimals) || 18;
        return {
            ok: true,
            address: holder,
            token: dngToken,
            symbol: String(symbol || 'DNG'),
            decimals: scale,
            // Both forms: `raw` for anyone who wants to do their own arithmetic, and the display
            // number the page prints. Nothing is valued on the float.
            raw: String(raw),
            amount: Number(utils.formatUnits(raw, scale)),
        };
    } catch (error) {
        console.warn(`[staking-chain] the $DNG balance did not decode: ${error.message || error}`);
        return { ok: false, reason: 'the token answered something this reader cannot read' };
    }
}
...
export async function readOwnedKnights(address, {
    ...
    if (!candidates.length) {
        return {
            ...
            // Not probed on this path: there was nothing to ask about. The field is absent on
            // purpose and the callers that print it read `shape` as unknown rather than as a claim.
            fromBlock,
            note: balance === 0
    ...
    const shape = await infoShapeFor(candidates, nftAddress);
    if (!shape) {
        return { ok: false, reason: 'the collection could not be read just now' };
    }
    const infoFn = shape === 'genesis' ? 'getGenesisKnightInfo' : 'getKnightInfo';
    ...
    for (let start = 0; start < candidates.length; start += size) {
        ...
        try {
            rows = await rpcBatch(chunk.map((tokenId) => ({
                ...
                params: [{
                    to: nftAddress,
                    data: NFT_IFACE.encodeFunctionData(infoFn, [tokenId]),
                }, 'latest'],
            })));
        } catch (error) {
        ...
        chunk.forEach((tokenId, index) => {
            ...
            if (shape === 'genesis') {
                ...
                let bandIndex;
                try {
                    [transferOwner, hashPower, bandIndex] = NFT_IFACE.decodeFunctionResult('getGenesisKnightInfo', row.result);
                } catch {
                    return;
                }
                ...
                knights.push({
                    tokenId,
                    name: `Genesis Knight #${tokenId}`,
                    // No tier, and that is a fact about the collection rather than a missing read:
                    // a Genesis knight carries hash power instead. `null` keeps that distinction,
                    // because 0 or 'common' would both be claims the contract never made.
                    rarity: null,
                    tierName: null,
                    hashPower: Number(hashPower),
                    bandIndex: Number(bandIndex),
                });
                return;
            }
    ...
    return {
        ...
        enumerable: isEnumerable,
        fromBlock,
        // Which question this collection answered, so a caller can say which shape it is showing
        // instead of inferring it from a missing field.
        shape,
