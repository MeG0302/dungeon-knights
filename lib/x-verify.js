 ...
 */
/**
 * Does the post carry an expected phrase?
 *
 * Loose on the shape, strict on the number: `Day 3`, `day3`, `DAY  3` and a marker wrapped in
 * punctuation are all the same claim, while `Day 30` must not satisfy `Day 3` — a player retyping
 * the marker by hand is not cheating, and an extra digit is a different day.
 */
export function containsMarker(text, marker) {
    const wanted = String(marker || '').trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\s+/g, '\\s*');
    if (!wanted) return false;
    return new RegExp(`(^|[^\\w])${wanted}(?![\\w])`, 'i').test(String(text || ''));
}
 ...
 *   - `marker`   a phrase the post has to carry, e.g. `Day 3` (the share's freshness rule)
 ...
 *
 * `marker` exists because the daily share can be asked for once a day, and the cheapest way to be
 * paid twice for one post is to paste **yesterday's link** today. Every day has its own marker, so
 * a post that does not carry today's is not today's post — and the refusal says which word was
 * missing, because the share box hands the player the text with it already in.
 *
...
export async function verifyPost(input, options = {}) {
    ...
    // Today's marker, before the link: a post from an earlier day of the program is the one thing a
    // daily reward is most likely to be asked for twice, and the sentence a player reads should say
    // which word was missing rather than that the link was wrong.
    if (expect.marker && !containsMarker(text, expect.marker)) {
        return {
            ok: false,
            code: 'missing-marker',
            reason: `That post does not say "${expect.marker}". Today's post text carries it — post that and file the new link.`,
            post,
        };
    }
