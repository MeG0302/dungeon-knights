 ...
 * link, and a share of a finished vault run. Paying on trust is not an option (the browser is
 * the claim, never the proof), and in February 2026 X closed its free API tier and moved
 * everyone to per-request credits — so reading a post through the official API costs money per
 * check, for every player, forever.
 *
 ...
 * ------------------------------------------------------------------------------
 * It proves a post **exists**, who **wrote** it, and what it **says**. It cannot prove a *like*
 * or a *retweet*: oEmbed has no view of either, and claiming otherwise would be the exact kind
 * of guess this project's other checks exist to prevent. Those two need X API credits or a
 * zkTLS proof (Reclaim / TLSNotary), which is a dependency this file deliberately does not have.
 *
...
export async function verifyPost(input, options = {}) {
    ...
    return { ok: true, post: { ...post, mentionsCampaign, keyword: wantedKeyword } };
}
