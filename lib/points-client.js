...
/** Hand the X account back. Points already earned stay. */
export async function requestUnbindX() {
    const session = readSession();
    if (!session) throw new Error('not signed in');
    return api('/api/points/x', { method: 'POST', token: session.token, body: { action: 'unbind' } });
}
