...
export async function signIn(address) {
    // The seam can be published before it holds anything. `privyBridgeReady` and
    // `privyAuthChanged` come from React effects, and the wallet behind them is adopted a
    // moment later — so a sign-in that arrives in that gap used to answer "no wallet found…
    // install MetaMask" to a player who was already signed in, naming the one thing a phone
    // cannot do. `provider({ waitMs })` is the seam's own give-it-a-moment, and it installs the
    // wallet as it adopts, which is what `activeProvider()` reads next.
    if (!activeProvider() && window.DKWallet?.provider) {
        await window.DKWallet.provider({ waitMs: 1500 });
    }
...
// ------------------------------------------------------------- the daily draw
/**
 * The giveaway as the server sees it: today's board, the last draw, and — when this browser holds a
 * session — the caller's own day, their capsules and the one waiting to be claimed.
 *
 * Asked for even by signed-out visitors, because the board a prize is decided from is public.
 */
export async function fetchGiveaway() {
    const session = readSession();
    const res = await fetch('/api/points/giveaway', {
        headers: session ? { Authorization: `Bearer ${session.token}` } : {},
        cache: 'no-store',
    });
    if (!res.ok) throw new Error('Could not load the capsule draw.');
    return res.json();
}

/**
 * Sign one of the capsule messages, through the same provider the session uses.
 *
 * Split out of `signIn` rather than duplicated: a capsule signature and a sign-in signature differ in
 * what the server writes, never in how the wallet is reached, and two copies of "find the provider,
 * wait for the bridge, ask for a signature" is how one of them ends up unable to sign on a phone.
 */
async function signWithWallet(message, address) {
    // The seam can be published before it holds anything — see the note in `signIn`.
    if (!activeProvider() && window.DKWallet?.provider) {
        await window.DKWallet.provider({ waitMs: 1500 });
    }
    const provider = activeProvider();
...
/** A message to sign that proves this wallet, so a capsule can be sent to it. */
export async function requestCapsuleRegister() {
    const session = readSession();
    if (!session) throw new Error('not signed in');
    const challenge = await api(`/api/points/capsule?purpose=register`, { token: session.token });
    const signature = await signWithWallet(challenge.challenge.message, session.address);
    return api('/api/points/capsule', {
        method: 'POST',
        token: session.token,
        body: { action: 'register', message: challenge.challenge.message, signature },
    });
}

/**
 * Claim one day's capsule.
 *
 * The day is asked about and then signed for, so the message the wallet shows names the capsule being
 * claimed rather than a general permission — and the server compares the day it was signed for with
 * the day being claimed, which is what makes submitting daily a rule rather than a suggestion.
 */
export async function requestCapsuleClaim(day) {
    const session = readSession();
    if (!session) throw new Error('not signed in');
    const query = day ? `?purpose=claim&day=${encodeURIComponent(day)}` : '?purpose=claim';
    const challenge = await api(`/api/points/capsule${query}`, { token: session.token });
    const signature = await signWithWallet(challenge.challenge.message, session.address);
    return api('/api/points/capsule', {
        method: 'POST',
        token: session.token,
        body: { action: 'claim', day: challenge.challenge.day, message: challenge.challenge.message, signature },
    });
}

/**
 * Say that the wallet has been handed over in the form.
 *
 * Self-declared, and named that way on the card: the form is a Google Form this project cannot read,
 * so the marker is what the team checks the responses against rather than a receipt.
 */
export async function markCapsuleForm(day) {
    const session = readSession();
    if (!session) throw new Error('not signed in');
    return api('/api/points/capsule', { method: 'POST', token: session.token, body: { action: 'form', day } });
}

// ------------------------------------------------------------------ redeeming points
/**
 * The wheel's state for the wallet this browser is signed in with: balance, what is left in each
 * card's pool, and the spins this address has already played — its own codes included. The pool
 * itself is not in there, at any depth; the server sends counts and nothing else.
 */
export async function fetchRedeem() {
    const session = readSession();
    if (!session) {
        const error = new Error('not signed in');
        error.status = 401;
    ...
    return (await api('/api/points/redeem', { token: session.token })).state;
}

/**
 * One spin, for one spin id.
 *
 * `spinId` is a uuid this browser makes up for this spin and sends with it. It is what makes a retry
 * safe: a request that arrives twice is answered from the wallet's own record — the same outcome and
 * the same code, once charged. Refusals arrive as errors the page switches on: `not_enough_points`
 * and `out_of_stock` come back as a 400 carrying fresh `state`, and `busy` as a 409.
 */
export async function requestSpin(spinId) {
    const session = readSession();
    if (!session) throw new Error('not signed in');
    return api('/api/points/redeem', {
        method: 'POST',
        token: session.token,
        body: { action: 'spin', spinId },
    });
}
