...
/**
 * Set by the middleware when it strips a callback, so the page can say so.
 *
 * There is no way to tell an interrupted link of ours from somebody else's pasted URL — both
 * arrive with no marker — which is why the sentence the page shows is conditional. Silence is
 * the alternative, and silence is what makes a player press *Authorize* on X and then believe
 * the site ignored them.
 */
export const DROPPED_COOKIE = 'dk_privy_dropped';

/**
 * Where the version before this one kept "we started a flow".
 *
 * It lived in `sessionStorage`, which the server cannot read — the whole reason the marker is
 * a cookie now. Nothing reads these any more; they are listed so the leftovers of that version
 * can be cleared instead of sitting in every browser that used it, and so that a fresh one
 * (a tab duplicated mid-flight, where sessionStorage is shared) is promoted into the cookie
 * rather than thrown away.
 */
export const LEGACY_MARKER_KEY = 'dk:privy:flow-started';
export const LEGACY_SEEN_KEY = 'dk:privy:oauth-seen';
...
/**
 * Clear what the previous version of this guard left behind, and keep the one piece worth
 * keeping.
 *
 * Honest about its own value: promoting the old marker only helps in the narrow case where the
 * same `sessionStorage` is visible to a page running this code (a duplicated tab, or a browser
 * that restored the tab into the new build). The *cleanup* is the point — a stale "a flow is in
 * flight" flag with no reader is exactly the kind of thing that misleads the next person to
 * touch this file. Never throws, and never writes when there is nothing to promote.
 */
export function migrateLegacyMarker({
    ...
    try {
        if (!storage || !doc) return { promoted: false, cleared: false };

        const legacy = storage.getItem(LEGACY_MARKER_KEY);
        let promoted = false;
        if (legacy && !readMarkerCookie(doc.cookie)) {
            const age = now - Number(legacy);
            if (Number.isFinite(age) && age >= 0 && age <= MARKER_TTL_MS) {
                promoted = markPrivyFlowStarted({ document: doc, now });
            }
        }

        storage.removeItem(LEGACY_MARKER_KEY);
        storage.removeItem(LEGACY_SEEN_KEY);
        return { promoted, cleared: true };
    } catch {
        // Storage off, or something else's idea of sessionStorage: nothing to do.
        return { promoted: false, cleared: false };
    }
}

/**
 * The note the middleware left when it dropped a callback, taken once.
 *
 * Reading it clears it: a player is told the link did not finish on the load it happened on,
 * and not on every page they open afterwards.
 */
export function consumeDroppedNotice({
    document: doc = typeof document !== 'undefined' ? document : null,
    location: loc = typeof window !== 'undefined' ? window.location : null,
    ...
    try {
        if (!doc) return null;
        const match = String(doc.cookie || '').match(new RegExp(`(?:^|; )${DROPPED_COOKIE}=([^;]*)`));
        if (!match) return null;

        // The middleware always sets this one `Secure` (it is https that serves it), and a browser
        // refuses to let a non-secure Set-Cookie clobber a secure cookie — so the deletion has to be
        // secure too, or the note comes back on every page load. `isSecureContext` is what covers
        // `http://localhost`, where the server's Secure attribute is still honoured.
        const secure = String(loc?.protocol || '') === 'https:' || isSecureContext ? '; Secure' : '';
        doc.cookie = `${DROPPED_COOKIE}=; Path=/; Max-Age=0; SameSite=Lax${secure}`;
        return { dropped: true, reason: decodeURIComponent(match[1] || '') || 'not-started-here' };
    } catch {
        return null;
    }
}
...
export function installOauthReturnGuard({
    ...
    try {
        ...
        // Two leftovers from the version this replaced: its marker, and its record of a code it
        // had already handled. Cleared here because this is the first thing that runs on the new
        // build, and because a page must not depend on either of them any more.
        migrateLegacyMarker({ document: doc, now });
